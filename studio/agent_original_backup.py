...
class State(TypedDict):
    ...
    dataset_profile: JSONType  # Step 0.1: Read the dataset
...
def generate_visualization_queries(state: State):
    ...
    dataset_profile_json = json.dumps(dataset_profile, indent=2, ensure_ascii=False)
    ...
    response_content = invoke_llm_with_prompt(
        ...
        replacements={
            ...
            "{{attributes}}": str(dataset_profile.keys()),  # type: ignore
        },
    )
...
def create_workflow():
    ...
    builder.add_edge(START, "generate_visualization_queries")
    builder.add_edge(
...
class Agent:
    ...
    def initialize_state_from_csv(self) -> dict:
        ...
        save_json_data(dataset_profile, "00_dataset_profile.json", "./datasets")
    ...
    def decode_output(self, output: dict):
        ...
        if "generated_research_paper_components" in output:
            ...
            self.generate_research_paper_html_report(
                output["generated_research_paper_components"], "output.html"
            )
        else:
            print("Using original HTML generation...")
            generate_html_report(output, "output.html")

    def generate_research_paper_html_report(
        self, research_paper_components: dict, output_path: str
    ):
        """
